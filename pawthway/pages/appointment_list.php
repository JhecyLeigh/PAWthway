...
$pending_sql = "SELECT * FROM appointments WHERE user_id = ? AND (status = 'Pending' OR status = 'Cancelled') ORDER BY appointment_date DESC";
$completed_sql = "SELECT * FROM appointments WHERE user_id = ? AND status = 'Confirmed' ORDER BY appointment_date DESC";
...
.edit-btn:hover { background: #43a047; }
              ...
              <div class="action-btns">
                <?php if ($app['status'] == 'pending'): ?>
                  ...
                  <form method="POST" style="display:inline;">
                    <input type="hidden" name="appointment_id" value="<?= $app['id']; ?>">
                    <button type="submit" name="cancel_appointment" class="btn cancel-btn">Cancel</button>
                  </form>
                <?php else: ?>
                  <em>No actions</em>
                <?php endif; ?>
